...
def calc_e_dose(t, e_dose, on_off = c.on_off, off_set=c.off_set, days_on = c.days_on, days_wait=c.days_wait):
    """
        Params:
    ...
    """
    if not isinstance(t, np.ndarray) and t <= days_wait:
        return 0
    t = t+off_set
    if on_off:
...
def calc_p_dose(t, p_dose, on_off = c.on_off, off_set=c.off_set, days_on = c.days_on, days_wait=c.days_wait):
    """
        Params:
            t (either float or np.array)
            p_dose (float)  
...
def solve(days_on=c.days_on):
    ...
    yy = ddeint(derivs, initial_conditions, tt, fargs=(params,days_on,))
    return tt, yy 
...
def get_variable(var_name, yy, tt, p, days_on):
    if var_name == 'E_2':
        yy = calculate_E2(p, yy, tt, days_on)
    elif var_name == 'P_4':
        yy = calculate_P4(p,yy,tt, days_on)
...
def plot_four_variables(yy, var_names, tt, num_samples = c.num_samples, on_off = c.on_off, days_on = c.days_on):
